...
방구석 세계여행 v. 2.0 beta for ubuntu
...
'''
    ...
    def mainWindow(self): #GUI Window 담당
        ...
        #상단 라벨
        label = tk.Label(main, text='Wherever you want...', font = ('', 12))
        #도시 입력 엔트리
        destination = tk.Entry(main, width = 22)
        #내 위치 찾기 버튼
        btn_myLoc = tk.Button(main, text = 'My Location', width = '9', height = '1')
        ...
        driv_rd.flash()
        none_rd = tk.Radiobutton(main, text="Nothing to add", value=3, variable=search_option_radio)
        none_rd.config(command = search_option)
        #이륙 버튼 (시작)
        btn_takeOff = tk.Button(main, text = 'TAKE OFF',width = '20', height = '2', bg = 'lightblue',\
        ...
        none_rd.place(x=170, y=90, anchor = 'w')
